...
${INPUT_DIR}/chr${chr}_SS_1.vcf.gz | \
awk '!seen[$1":"$2]++' > ${OUTPUT_DIR}/chr${chr}_unique_sites.txt

bcftools view \
-T ${OUTPUT_DIR}/chr${chr}_unique_sites.txt \
${INPUT_DIR}/chr${chr}_SS_1.vcf.gz \
-Oz -o ${OUTPUT_DIR}/chr${chr}_SS_1.clean.vcf.gz

bcftools view \
-T ${OUTPUT_DIR}/chr${chr}_unique_sites.txt \
${INPUT_DIR}/chr${chr}_SS_2.vcf.gz \
-Oz -o ${OUTPUT_DIR}/chr${chr}_SS_2.clean.vcf.gz
...
bcftools query -f '[%GT\t]\n' \
${OUTPUT_DIR}/chr${chr}_SS_1.clean.vcf.gz | \
awk '
{
    for(i=1; i<=NF; i++) {
        split($i,a,"|");
...
}' > ${OUTPUT_DIR}/chr${chr}_SS_1.hap

# Subopulation 2

bcftools query -f '[%GT\t]\n' \
${OUTPUT_DIR}/chr${chr}_SS_2.clean.vcf.gz | \
awk '
{
    for(i=1; i<=NF; i++) {
        split($i,a,"|");
